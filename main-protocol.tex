...
\item[$\bullet$]  if $I_{\scriptscriptstyle i}=\bot$, then does nothing. %then it increments  $y'_{\scriptscriptstyle\mathcal C}$ or $y'_{\scriptscriptstyle\mathcal S}$ by $1$, if the arbiter is invoked by the client or server respectively. 
\end{enumerate}
\end{enumerate}
