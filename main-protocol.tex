...
\item Calls  $\mathtt{VSID.setup}(1^{\lambda}, u,k,M)\rightarrow (u^{\scriptscriptstyle *}, \textcolor{purple}{pp},e)$, to encode  service input, and generate metadata. \textcolor{purple}{It appends $pp$ to $qp$}.
...
\item If any above check is rejected, then it sets $a=0$. Otherwise, it  calls $\mathtt{VSID.serve}(u^{\scriptscriptstyle *},e,pk, \textcolor{purple}{pp})\rightarrow a$. 
...
\item\label{arbiter-identify} calls   $\mathtt{VSID.identify}(\pi''_{\scriptscriptstyle i},c_{\scriptscriptstyle i}, k, e', \textcolor{purple}{pp})\rightarrow I_{\scriptscriptstyle i}$
\begin{enumerate}
...
\begin{remark}
In phase \ref{RCSP::Dispute-Resolution}, vector $\bm{des}$ is used to let the arbiter discard a party's complain on a certain verification, say $i$-th, (without processing that request) when the arbiter has already identified a misbehaving party for that verification, i.e. already output $I_{i}\in\{\mathcal{C}, \mathcal{S},\bot\}$. It ensures that the arbiter does not run the same request multiple times, so its resources would not be wasted. 
\end{remark}
