...
\begin{proof}[sketch]
Due to the privacy property of SAP, that stems from the  hiding property of the commitment scheme, given the commitments $g_{\scriptscriptstyle qp}$ and $g_{\scriptscriptstyle cp}$, (that are stored in the blockchain as result of running SAP) the adversary learns no information about the committed values (e.g. $o, l, pad_{\scriptscriptstyle\pi},pad_{\scriptscriptstyle q},$ and $\bar{k}$), except with negligible probability, $\mu(\lambda)$.   Also, given encoded coins $coin^{\scriptscriptstyle *}_{\scriptscriptstyle\mathcal{C}}=z\cdot (o_{\scriptscriptstyle max}+l_{\scriptscriptstyle max})$ and  $coin^{\scriptscriptstyle *}_{\scriptscriptstyle\mathcal{S}}=z\cdot l_{\scriptscriptstyle max}$, the adversary learns nothing about the actual price agreed between the server and client,  $(o,l)$.  In the following, we argue why it is the case. According to the game in Definition \ref{deff::RC-S-P-Input-Privacy},  we let the adversary pick a pair of  service input, $(u_{\scriptscriptstyle 0},u_{\scriptscriptstyle 1})$. Let  $(o_{\scriptscriptstyle i}, l_{\scriptscriptstyle i})$  be actual price pair corresponding to $u_{\scriptscriptstyle i}$,   where $i\in\{0,1\}$. Also, for the sake of simplicity, let the price list be $\{(o_{\scriptscriptstyle 0}, l_{\scriptscriptstyle 0}),(o_{\scriptscriptstyle 1}, l_{\scriptscriptstyle 1})\}$. Note that in this case, $o_{\scriptscriptstyle max}=max(o_{\scriptscriptstyle 0},o_{\scriptscriptstyle 1})$ and $l_{\scriptscriptstyle max}=max(l_{\scriptscriptstyle 0},l_{\scriptscriptstyle1})$. Let $\beta\in\{0,1\}$ be the service input index  picked by the experiment. As it is evident, the value of $coin^{\scriptscriptstyle *}_{\scriptscriptstyle\mathcal{C}}$  is independent of the experiment's choice, $\beta$. In fact, it is a function of the maximum price pair in the price list (and a public value $z$). Therefore, given $coin^{\scriptscriptstyle *}_{\scriptscriptstyle\mathcal{C}}$ the adversary cannot tell with a probability significantly grater than $\frac{1}{2}$ which price pair or service input has been picked by the experiment. The same holds for  $coin^{\scriptscriptstyle *}_{\scriptscriptstyle\mathcal{S}}$, as its value is independent of $\beta$ as well. 
...
%Also, for the sake of simplicity, let the price list be $\{(o_{\scriptscriptstyle 0}, l_{\scriptscriptstyle 0}),(o_{\scriptscriptstyle 1}, l_{\scriptscriptstyle 1})\}$. Note that in this case $max=1$. 
